...
- **`Failed to parse data from bookmarklet`** → the bookmarklet wasn't clicked on the Teams tab (so localStorage didn't contain MSAL entries). Re-run `teams login`, make sure you're signed in and on the Teams chat view, then click the bookmark again.
